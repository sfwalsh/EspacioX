...
final class LaunchListDefaultInteractor: LaunchListInteractor {
    ...
    func fetchNextLaunch(completion: @escaping ((NetworkResult<Launch>) -> Void)) {
        guard let request = API.buildNextLaunchRequest() else {
            completion(.failure(error: LaunchFetchError.requestError))
            return
        }
        ...
        URLSession.shared.dataTask(with: request) { [weak self] (data, response, error) in
            ...
            guard let data = data,
                let launches = self?.parseUpcomingLaunchResponseData(data: data) else {
                    completion(.failure(error: LaunchFetchError.jsonDecoding))
                    return
            }
    ...
    private func parseUpcomingLaunchResponseData(data: Data) -> Launch? {
        return try? JSONDecoder().decode(Launch.self, from: data)
    }
}
